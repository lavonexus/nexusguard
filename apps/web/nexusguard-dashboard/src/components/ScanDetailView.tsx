...
// Structural subset both ScanSessionResponse (customer, per-server) and AdminScanSummaryResponse
// (site admin, cross-server) satisfy - this view only ever reads these fields off either shape,
// so it works unchanged for both without a shared base DTO on the backend.
...
export interface ScanDetailLike<TSummary extends ScanSummaryLike> {
  session: TSummary;
...
// Firmware/raw sections, accounts card) - shared between the customer-facing /scans/[id] page
// and the site admin's /admin/scans/[id] page. Both pages fetch their own way (per-server API
// key vs. site-admin session) and hand this component the same-shaped detail once loaded.
export default function ScanDetailView<TSummary extends ScanSummaryLike>({
  detail,
  pin,
}: {
  detail: ScanDetailLike<TSummary>;
  pin: string | null;
}) {
...
// Sidebar category navigation. Sections with nothing to show (no RPF/USB/firmware facts) are
...
function OverviewSection<TSummary extends ScanSummaryLike>({
  detail,
  pin,
  systemFact,
  scanDuration,
...
function AccountsCard<TSummary extends ScanSummaryLike>({ session, t }: { session: TSummary; t: T }) {
