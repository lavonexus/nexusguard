  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white">Günaydın, {displayName}.</h1>
          <p className="mt-1 text-sm text-zinc-400">İşte tespit özetin.</p>
        </div>
        <button
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Tile label="Toplam tarama" value={stats?.total} delta={stats?.totalDelta} />
        <Tile label="Tespitler" value={stats?.cheating} tone="danger" />
        <Tile label="Temiz taramalar" value={stats?.clean} tone="good" />
        <Tile label="Tespit oranı" value={stats ? `${stats.detectionRate.toFixed(1)}%` : undefined} />
      </div>

      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-5 lg:col-span-2">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-zinc-200">Tarama etkinliği</h2>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-5">
          <h2 className="text-sm font-semibold text-zinc-200">Karar dağılımı</h2>
          <div className="mt-4">
            {stats ? (
              <DecisionDonut
                buckets={[
                  { label: "Temiz", value: stats.clean, color: DECISION_COLORS["Temiz"] },
                  { label: "Uyarı", value: stats.suspicious, color: DECISION_COLORS["Uyarı"] },
                  { label: "Hile", value: stats.cheating, color: DECISION_COLORS["Hile"] },
                  { label: "Çalışıyor", value: stats.pending, color: DECISION_COLORS["Çalışıyor"] },
                ]}
              />
            ) : (
              <ChartSkeleton />
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/30 lg:col-span-2">
          <div className="flex items-center justify-between px-5 py-4">
            <h2 className="text-sm font-semibold text-zinc-200">Son taramalar</h2>
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-t border-zinc-800 bg-zinc-900/60 text-zinc-500">
              <tr>
                <th className="px-5 py-2 font-medium">Oyuncu</th>
                <th className="px-5 py-2 font-medium">Durum</th>
                <th className="px-5 py-2 font-medium">Risk skoru</th>
                <th className="px-5 py-2 font-medium">Ne zaman</th>
              </tr>
            </thead>
              ...
              {recentScans.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-5 py-6 text-center text-zinc-500">
                    {scans === null ? "Yükleniyor..." : "Henüz tarama yok."}
                  </td>
                </tr>
              )}
              {recentScans.map((s) => (
                ...
                >
                  ...
                  <td className="px-5 py-2.5 text-zinc-500">{new Date(s.createdAt).toLocaleString("tr-TR")}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-5">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-zinc-200">Üyelik</h2>
            ...
            <p className="mt-2 text-xs text-zinc-500">
              {server?.plan === "Enterprise" ? "Aktif abonelik." : "Ücretsiz plandasın."}
            </p>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-5">
            <h2 className="text-sm font-semibold text-zinc-200">Sistem durumu</h2>
            <div className="mt-3 space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-zinc-400">API</span>
                <span className={error ? "text-red-400" : "text-emerald-400"}>{error ? "ÇEVRİMDIŞI" : "ÇEVRİMİÇİ"}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-zinc-400">Web sitesi</span>
